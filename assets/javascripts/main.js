...
function chat_init() {
  ...
  $('#chatArea').on('mouseover', function(event) {
    newMessage = 0;
  });
}
...
function submitMsg() {
  ...
  $.post(
    ...
    function() {
      $.post( 
      	redmineRootPath + '/chat/receive_chat', 
        function(data) {
...
function refreshChat() {
    preContent = '0';
    initializer = 2;
    $('#chatMessages').PeriodicalUpdater(redmineRootPath + '/chat/receive_chat', 
    	{
        method: 'post',
        multiplier: 1,
        minTimeout: 5000,
      ...
      function() {
        $('#ajax-indicator').css('visibility', "visible");
...
$(function() {
    ...
    window.flashMsg = function(newMsg) {
      function step() {
        ...
        $('chatFooter').html(($('chatFooter').html() == "Chat") ? newMsg : "Chat");
        if (newMessage == 1) {
          timeout = setTimeout(step, 1000);
        } else {
          $('chatFooter').html("Chat");
